...
angular.module("FairyDemo").run(["$templateCache", function($templateCache) {$templateCache.put("usageExamples/simpleIntro.html","<div ng-controller=\"example1Controller as example1Controller\"><div ng-if=\"!example1Controller.clickedButton\"><p>When a new user opens your app it should be very clear what they should do first. This makes it easier for people to understand what\'s happening and quickly do something \"right.\"</p><p>Adding a fairy to this action is cute and friendly, and also associates the fairies with actions -\nsafe, useful actions. This initial fairy isn\'t necessary to teach a new person how to do the right action,\nbecause that\'s already so simple that no one could miss it. The first fairy is just to teach people what the\nfairies do, what you can expect out of them, and that they can be trusted.</p><center><button ng-click=\"example1Controller.clickedButton = true\" guide-stop=\"example-1-action\" guide-on-fairy-tickle=\"example1Controller.clickedButton = true\" guide-fairy-class=\"fairy--read-on-button\" class=\"btn btn-primary btn-lg\">There\'s only one thing to do</button><p class=\"small\">(and it teaches you what fairies are even if you won\'t read my copy)</p></center></div><div ng-if=\"example1Controller.clickedButton\"><p>Here\'s what the code looks like.</p><pre class=\"prettyprint\">&lt;button guide-stop=\'example-1-action\'&gt;\n    There\'s only one thing to do\n&lt;/button&gt;\n</pre><pre class=\"prettyprint\">guideFairies.showStop(\'example-1-action\'); // this stop id matches\n                                           // the guide-stop value</pre></div></div>");}]);